...
public class RichMenuHandler implements CommandLineRunner {
	...
	public String createRichMenu() throws InterruptedException, ExecutionException {
		...
		try {
			richMenu = RichMenu.builder().name("LYMenu").areas(richMenuAreas()).chatBarText("Liy Server").selected(true)
		...
		return lineBotClient.createRichMenu(richMenu).get().getRichMenuId();
	}
	...
	private static List<RichMenuArea> richMenuAreas() throws URISyntaxException {
		...
		return Arrays.asList(richMenuArea(0, 0, "系統管理主頁", "https://liff.line.me/2000468524-BGX34p74"),
	...
	private static RichMenuArea richMenuArea(final int xn, final int yn, String name, String actionUrl)
			throws URISyntaxException {
		final int X_STEP = 883;
		final int Y_STEP = 843;

		return new RichMenuArea(new RichMenuBounds(X_STEP * xn, Y_STEP * yn, X_STEP, Y_STEP),
				new URIAction("LiffLogin", new URI(actionUrl), new AltUri(new URI(actionUrl))));
	}
...
//		若沒有資料，就creat新的，檢查使用者有沒有menu，若無，就要設定menu。
		...
		setBackGroundToRichMenu(richMenuId);
//		combineMenuAndUser(richMenuId);
	 ...
	 * 更新圖片自己組電文去打
	 */
	private void setBackGroundToRichMenu(String richMenuId) {
	}

//	private void linkRichMenuToUser(String richMenuId) {
//
//	}
}
